...
import { Star, Car, Phone, MapPin, Clock, Award, Calendar, ChevronRight, Edit3, Settings } from 'lucide-react'
...
export default function DriverProfile() {
  ...
  return (
    <div className="flex-1 px-4 py-4 space-y-4">
      ...
      {/* Settings button (for drivers viewing own profile) */}
      {user?.role === 'DRIVER' && user?.id === selectedDriver?.id && (
        <Button
          onClick={() => setView('settings')}
          className="w-full h-12 rounded-2xl bg-white border-2 border-[#006233] text-[#006233] hover:bg-[#006233]/5 font-semibold shadow-lg gap-2"
        >
