...
import { getNearestLocation } from '@/lib/geo'
...
export default function PassengerSearch() {
  ...
  const { location: geoLocation, loading: geoLoading, error: geoError, refresh: geoRefresh } = useGeolocation()

  // Auto-select nearest origin when geolocation succeeds
  useEffect(() => {
    if (geoLocation && !origin) {
      const nearest = getNearestLocation(geoLocation.latitude, geoLocation.longitude)
      if (nearest) {
        setOrigin(nearest)
        toast.success(`Position détectée : ${nearest}`)
      }
    }
  }, [geoLocation])
  ...
  return (
    <div className="flex-1 px-4 py-4 space-y-5">
      ...
      <button
        onClick={geoRefresh}
        disabled={geoLoading}
        className="w-full py-3 rounded-2xl text-sm font-semibold transition-all min-h-[48px] bg-white border-2 border-dashed border-[#006233]/30 text-[#006233] hover:border-[#006233] hover:bg-[#006233]/5 active:scale-95 flex items-center justify-center gap-2"
      >
        {geoLoading ? (
        ...
        {geoLoading ? 'Localisation...' : geoError ? geoError : '📍 Ma position'}
      </button>
